...
\textbf{Backend API Service}: A FastAPI-based REST API serving image captioning requests using the Salesforce BLIP model~\cite{baylor2017tfserving}. The service handles file uploads, performs inference, and returns generated captions with timing metrics.

\textbf{Frontend Testing Application}: A React/TypeScript web application providing file upload, experiment configuration, concurrent request submission, and real-time metrics visualization. The frontend enables controlled experimentation across different platforms and scaling configurations.
...
\textbf{Root Cause}: C-extension libraries (Intel MKL, OpenBLAS) detect total system vCPUs at import time and configure thread pools accordingly, unaware of other worker processes competing for the same resources~\cite{intel2023mkl}.
...
\item \textit{Timeout configuration}: Override default 30-second idle timeout to 300 seconds to accommodate long-running inference requests (2-5s per request)
\end{itemize}

Port misconfiguration between ECS Task, Target Group, and Listener causes requests to fail silently with connection timeouts. Additionally, default timeout values (30s) terminate active inference requests prematurely, requiring explicit configuration adjustments.

\textbf{Azure Simplicity}: Azure Container Apps abstracts all load balancing complexity. Scaling from 1 to 4 replicas requires only changing the replica count parameter—no target groups, listeners, port mappings, or timeout overrides. The platform manages load distribution, health checks, and timeouts internally with sensible defaults for containerized applications.
...
\textbf{Solution}: For AWS, we manually configure Application Load Balancer (ALB) including Target Groups, Listeners, port mappings, and timeout overrides to support long-running AI inference requests (Challenge 3). For Azure Container Apps, load balancing is managed internally by the platform when scaling to multiple replicas—no explicit configuration required. Table~\ref{tab:lb_config} summarizes the configuration.

\begin{table}[h]
\centering
\caption{Load Balancer Configuration (Horizontal Scaling)}
\label{tab:lb_config}
\begin{tabular}{lll}
\toprule
\textbf{Parameter} & \textbf{AWS ALB} & \textbf{Azure Container Apps} \\
\midrule
Algorithm & Round-robin & Built-in (automatic) \\
Health Endpoint & GET /api/health & GET /api/health \\
Health Interval & 30 seconds & Default (internal) \\
Unhealthy Threshold & 3 failures & Default (internal) \\
Healthy Threshold & 2 successes & Default (internal) \\
Request Timeout & 300 seconds & 300 seconds \\
\bottomrule
\end{tabular}
\end{table}

\textbf{Rationale}: AWS ALB requires explicit multi-step configuration (Challenge 3): creating Target Groups with health check parameters, configuring Listeners with port routing, ensuring ECS Task Definition port mappings match, and overriding default 30-second timeout to 300 seconds. Port mismatches between these interconnected resources cause silent failures. Azure Container Apps abstracts this complexity entirely—scaling to multiple replicas automatically activates load balancing with sensible defaults for containerized workloads. Both approaches achieve equivalent functionality (distributing requests, health checking, timeout handling), but AWS demands significantly more manual configuration effort.

\textbf{Validation}: Before each experiment, we verify load distribution by submitting 10 warm-up requests and confirming requests arrive at all instances/replicas through backend logs. This ensures load balancing functions correctly before actual measurements begin.
...
All configurations use standardized 4 vCPU / 8GB RAM instances on both platforms. Environment variables control worker count (1, 2, or 4 workers), request timeout (300 seconds), thread pool sizes for numerical libraries, and other runtime parameters. The frontend configures the appropriate endpoint: direct instance URL for worker scaling, load balancer URL for horizontal scaling.
